...
import { buildInvoiceHtml } from './invoice-template.js';
  ...
  // Live chat state
  let chatConversations = [];   // 1 dòng / khách, gộp từ toàn bộ chat_messages
    ...
    try {
      ...
      supabase.channel('realtime-chat')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, async (payload) => {
          ...
          if (row.sender === 'user') {
            showNotification('🗨️ Chat mới!', (row.user_name || row.user_email || 'Khách') + ': ' + row.message.slice(0, 60) + '...');
          }
          if (row.user_id === activeChatUserId) {
  ...
  function showNotification(title, body) {
    ...
    const tabTarget = title.includes('📦') ? 'orders' : (title.includes('🗨️') ? 'chat' : 'messages');
  ...
  // ─── TABS ─────────────────────────────────────────────────────────────────
  const TAB_NAMES = ['dashboard', 'products', 'orders', 'messages', 'chat', 'reviews', 'backup'];
  ...
  async function loadAll() {
    await Promise.all([loadProducts(), loadOrders(), loadMessages(), loadReviews(), loadChatConversations()]);
    updateStats();
  }
  ...
  async function loadChatConversations() {
    ...
    if (error) { console.error('loadChatConversations error:', error); return; }

    const byUser = new Map();
    for (const row of (data || [])) {
      let conv = byUser.get(row.user_id);
      if (!conv) {
        conv = { user_id: row.user_id, user_name: null, user_email: null, last_message: '', last_time: null, unread: 0 };
        byUser.set(row.user_id, conv);
      }
      if (row.user_name) conv.user_name = row.user_name;
      if (row.user_email) conv.user_email = row.user_email;
      conv.last_message = row.message;
      conv.last_time = row.created_at;
      if (row.sender === 'user' && !row.is_read) conv.unread++;
    }
    chatConversations = Array.from(byUser.values()).sort((a, b) => new Date(b.last_time) - new Date(a.last_time));
    renderChatConvList();
  }

  window.applyChatConvFilter = function () {
    chatConvFilter = (document.getElementById('chat-conv-search')?.value || '').trim().toLowerCase();
    renderChatConvList();
  };
  ...
  function renderChatConvList() {
    ...
    if (!el) return;
    const q = chatConvFilter;
    const list = q
      ? chatConversations.filter(c => (c.user_name || '').toLowerCase().includes(q) || (c.user_email || '').toLowerCase().includes(q))
      : chatConversations;

    if (!list.length) {
      el.innerHTML = `<div style="padding:20px;color:var(--muted);font-size:13px;text-align:center">Chưa có hội thoại nào.</div>`;
      return;
    }

    el.innerHTML = list.map(c => `
      <div class="chat-conv-item ${c.user_id === activeChatUserId ? 'active' : ''}" onclick="openChatConversation('${c.user_id}')">
        <div class="chat-conv-item-top">
          <span class="chat-conv-item-name">${escHtml(c.user_name || c.user_email || 'Khách')}</span>
          <span class="chat-conv-item-time">${formatChatTime(c.last_time)}</span>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
          <span class="chat-conv-item-preview">${escHtml(c.last_message)}</span>
  ...
  window.openChatConversation = async function (userId) {
    activeChatUserId = userId;
    renderChatConvList(); // cập nhật highlight item active

    const conv = chatConversations.find(c => c.user_id === userId);
    document.getElementById('chat-thread-header').textContent = conv ? (conv.user_name || conv.user_email || 'Khách') : 'Khách';
    document.getElementById('chat-thread-footer').style.display = 'flex';

    const box = document.getElementById('chat-thread-messages');
    box.innerHTML = `<div style="margin:auto;color:var(--muted);font-size:13px">Đang tải...</div>`;
    ...
    if (error) {
      box.innerHTML = `<div style="margin:auto;color:var(--red);font-size:13px">Lỗi tải tin nhắn.</div>`;
      return;
    }

    box.innerHTML = (data || []).map(chatBubbleHtml).join('');
    box.scrollTop = box.scrollHeight;

    // Đánh dấu đã đọc các tin của khách trong hội thoại này
  ...
  function chatBubbleHtml(row) {
    const cls = row.sender === 'user' ? 'from-customer' : 'from-admin-reply';
    const time = new Date(row.created_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' });
    return `<div class="chat-bubble ${cls}">${escHtml(row.message)}<span class="chat-bubble-time">${time}</span></div>`;
  }

  function appendChatBubble(row) {
    const box = document.getElementById('chat-thread-messages');
    if (!box) return;
    box.insertAdjacentHTML('beforeend', chatBubbleHtml(row));
    box.scrollTop = box.scrollHeight;
  }

  window.sendChatReply = async function () {
    ...
    if (error) { console.error('sendChatReply error:', error); alert('Không gửi được, thử lại.'); return; }
    input.value = '';
  };
  ...
  function updateStats() {
    ...
    setBadge('badge-chat',     chatConversations.reduce((sum, c) => sum + c.unread, 0));
  ...
  // Nén ảnh trước khi upload — resize về tối đa 1600px cạnh dài, chất lượng 82%.
  // Giảm dung lượng ảnh chụp thẳng từ điện thoại (thường vài MB) xuống chỉ còn
  // vài trăm KB, giúp trang load nhanh hơn nhiều (đặc biệt là giỏ hàng/trang chủ
  ...
  function compressImage(file, maxDim = 1600, quality = 0.82) {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => { URL.revokeObjectURL(url); reject(new Error('Ảnh lỗi, không đọc được')); };
      img.src = url;
    });
  }
  ...
  document.getElementById('p-img-files').addEventListener('change', async e => {
    ...
    // Chặn sớm file quá to (>25MB gốc) — tránh trình duyệt bị đứng lúc nén ảnh
    const MAX_ORIGINAL_MB = 25;
    const tooLarge = files.filter(f => f.size > MAX_ORIGINAL_MB * 1024 * 1024);
    if (tooLarge.length) {
      status.textContent = `❌ ${tooLarge.map(f => f.name).join(', ')} vượt quá ${MAX_ORIGINAL_MB}MB — vui lòng chọn ảnh nhỏ hơn.`;
      status.style.color = 'var(--red)';
      return;
    }

    // Giới hạn tối đa 8 ảnh/sản phẩm
    const MAX_IMAGES = 8;
    if (currentImages.length + files.length > MAX_IMAGES) {
      status.textContent = `❌ Mỗi sản phẩm tối đa ${MAX_IMAGES} ảnh (hiện có ${currentImages.length}, đang chọn thêm ${files.length}).`;
      status.style.color = 'var(--red)';
      return;
    }

    status.textContent = `⏳ Đang nén và upload ${files.length} ảnh...`;
    status.style.color = 'var(--muted)';

    let uploaded = 0;
    for (const file of files) {
      try {
        const compressed = await compressImage(file).catch(() => file); // nén lỗi thì fallback dùng ảnh gốc, không chặn upload
        const ext      = 'jpg';
        const fileName = `product-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
        const { error } = await supabase.storage.from('product-images').upload(fileName, compressed, { cacheControl: '3600', upsert: false });
  ...
  window.applyProductFilter = function(resetPage = true) {
  ...
  function renderProductPagination(totalPages) {
    ...
    let html = `<button class="edit-btn" ${currentProductPage === 1 ? 'disabled' : ''} onclick="goToProductPage(${currentProductPage - 1})">‹ Trước</button>`;
    for (let i = 1; i <= totalPages; i++) {
      // Chỉ hiện: trang đầu, trang cuối, trang hiện tại, và 1 trang liền kề mỗi bên — còn lại rút gọn "..."
      const showPage = i === 1 || i === totalPages || Math.abs(i - currentProductPage) <= 1;
      if (showPage) {
        html += `<button class="edit-btn" style="${i === currentProductPage ? 'background:var(--accent);color:#fff;border-color:var(--accent)' : ''}" onclick="goToProductPage(${i})">${i}</button>`;
      } else if (Math.abs(i - currentProductPage) === 2) {
        html += `<span style="padding:0 4px;color:var(--muted)">…</span>`;
      }
    }
    html += `<button class="edit-btn" ${currentProductPage === totalPages ? 'disabled' : ''} onclick="goToProductPage(${currentProductPage + 1})">Sau ›</button>`;
    wrap.innerHTML = html;
  }
  ...
  window.printInvoice = function(btn) {
    ...
    const html = buildInvoiceHtml(o, prod);
